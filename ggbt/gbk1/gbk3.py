...
class Rsimacd(bt.Indicator):
    ...
    def __init__(self):
        self.addminperiod(int(np.max([self.params.short, self.params.long, self.params.m,self.params.n,self.params.k1,self.params.k2])) + 2)
        # Add a KDJ indicator
        rsi = bt.ind.RSI(self.data.close,period=self.params.n)
        diff=(bt.ind.EMA(rsi,period=self.params.short)-bt.ind.EMA(rsi,period=self.params.long))/bt.ind.EMA(rsi,period=self.params.short)
        dea=bt.ind.EMA(diff,period=self.params.m)

        self.l.n1=bt.ind.EMA(diff-dea,period=self.params.k1)
        self.l.n2=bt.ind.EMA(diff-dea,period=self.params.k2)
...
opt = optunity.maximize(runstrat, num_evals=3, solver_name='particle swarm',
                        short=[1, 50], long=[1, 100],
                        m=[1,30],n=[1,30],k1=[1,15],k2=[1,30])
