...
def run1(s1, s2, l1, l2):
    ...
    try:
        ...
        winp = 0
        print('error')
    # return {'参数':[s1,s2,l1,l2],'交易次数':total,'胜率':winp,'最终价值':cerebro.broker.getvalue(),'年化收益':pnly}
    return pd.DataFrame([[s1,s2,l1,l2,total,winp,cerebro.broker.getvalue(),pnly]],columns=['s1','s2','l1','l2','times','win','value','pnly'])
    #return {'a': s1, 'b': s2, 'c': l1, 'd': l2, '交易次数': total, '胜率': winp, '最终价值': cerebro.broker.getvalue(), '年化收益': pnly}
    # return [[s1,s2,l1,l2],total,winp,cerebro.broker.getvalue(),pnly]
