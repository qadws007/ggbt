...
class KDJ(bt.Indicator):
    ...
    def __init__(self):
        self.addminperiod(self.params.period_kdj)
        # Add a KDJ indicator
...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):
        self.addminperiod(int(np.max([self.p.period_rsi,self.p.period_kdj,self.p.period_dfast,self.p.period_dslow])))
        # 保存收盘价的引用
        self.dataclose = self.datas[0].close
        # self.rsi = bt.indicators.RSI_SMA(
        #     self.data.close, period=self.p.period_rsi, upperband=self.p.short, lowerband=self.p.long)

        self.rsi=bt.indicators.RelativeStrengthIndex(self.data.close, period=int(self.p.period_rsi), upperband=int(self.p.short), lowerband=int(self.p.long))
        self.kdj=KDJ(period_kdj=int(self.p.period_kdj), period_dfast=int(self.p.period_dfast), period_dslow=int(self.p.period_dslow))
...
# 创建交易数据集
data = pd.read_csv(path_root + "/datas/600519.csv")
...
def runstrat(period_rsi,short,long, period_kdj,period_dslow,period_dfast):
    ...
    cerebro = bt.Cerebro()
    cerebro.addstrategy(TestStrategy, period_rsi=period_rsi, short=short,long=long,period_kdj=period_kdj,period_dslow=period_dslow,period_dfast=period_dfast)
...
opt = optunity.maximize(runstrat,  num_evals=3,solver_name='particle swarm',
                        period_rsi=[1,20], short=[1,100],long=[1,100],
                        period_kdj=[1,20],period_dslow=[1,20],period_dfast=[1,20])
