...
def CROSS_df(S1, S2):                     #判断向上金叉穿越 CROSS(MA(C,5),MA(C,10))  判断向下死叉穿越 CROSS(MA(C,10),MA(C,5))
    return (S1 <= S2).shift(1) & (S1 > S2)

def SMA_df(S, N, M=1):       #中国式的SMA,至少需要120周期才精确 (雪球180周期)    alpha=1/(1+com)
    return S.ewm(alpha=M/N,adjust=False).mean()           #com=N-M/M
